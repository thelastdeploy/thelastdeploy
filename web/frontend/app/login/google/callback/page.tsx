// web/frontend/app/login/google/callback/page.tsx
  ...
  useEffect(() => {
    ...
    async function authenticate(authCode: string) {
      try {
        ...
        const { access_token, device_key } = await api.loginWithGoogle(authCode, redirectUri);
        if (isMounted) {
    ...
    <div className="min-h-[calc(100vh-64px)] flex items-center justify-center p-6 bg-background">
      <div className="text-center space-y-4">
        ...
        <div className="space-y-1">
          ...
          <p className="text-muted-foreground text-sm">Completing authentication via Google...</p>
        </div>
      </div>
    </div>
  );
