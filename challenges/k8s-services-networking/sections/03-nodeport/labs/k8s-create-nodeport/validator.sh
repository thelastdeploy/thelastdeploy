#!/bin/bash
# validator.sh — k8s-services-networking / 03-nodeport / k8s-create-nodeport
set -euo pipefail

CONTEXT="kind-tld-k8s-create-nodeport"
...
if [ "$TYPE" != "NodePort" ]; then
  echo "FAIL: Service '$SVC' is of type '$TYPE', but it must be 'NodePort'."
  exit 1
fi
...
NODE_PORT=$(kubectl --context="$CONTEXT" get service "$SVC" -o jsonpath='{.spec.ports[0].nodePort}')
...
if [ "$NODE_PORT" != "30080" ]; then
  echo "FAIL: Service nodePort is '$NODE_PORT' (expected: '30080')."
  exit 1
fi
...
echo "PASS: NodePort service created successfully on nodePort 30080!"
exit 0
