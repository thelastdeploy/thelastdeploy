#!/bin/bash
# validator.sh — git-troubleshooting / 05-accidental-mistakes / git-undo-last-commit
set -euo pipefail
...
if [ "$COMMIT_COUNT" -ne 2 ]; then
  echo "FAIL: Repository has $COMMIT_COUNT commits (expected: 2). Did you undo the last commit?"
  exit 1
fi

# Verify three.txt is staged (cached)
if ! git -C "$REPO_DIR" diff --cached --name-only | grep -q "three.txt"; then
  echo "FAIL: File 'three.txt' is not staged. Did you use a soft reset?"
  exit 1
fi
...
echo "PASS: Last commit undone and changes kept in staging index!"
exit 0
